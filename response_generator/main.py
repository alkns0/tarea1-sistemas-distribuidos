...
def load_data():
    df = pd.read_csv(DATA_PATH, usecols=["latitude", "longitude", "area_in_meters", "confidence"])
    for zone_id, bbox in ZONES.items():
        ...
        print(f"Zona {zone_id}: {len(data[zone_id])} edificios cargados")

def zone_area_km2(zone_id):
    z = ZONES[zone_id]
    # Aproximación simple en km
    lat_diff = abs(z["lat_max"] - z["lat_min"]) * 111
    lon_diff = abs(z["lon_max"] - z["lon_min"]) * 111 * np.cos(np.radians((z["lat_max"] + z["lat_min"]) / 2))
...
def q1_count(zone_id: str, confidence_min: float = 0.0):
    ...
    count = sum(1 for r in records if r["confidence"] >= confidence_min)
    return {"zone_id": zone_id, "count": count, "confidence_min": confidence_min}
...
def q2_area(zone_id: str, confidence_min: float = 0.0):
    ...
    if not areas:
        return {"zone_id": zone_id, "avg_area": 0, "total_area": 0, "n": 0}
    return {"zone_id": zone_id, "avg_area": np.mean(areas), "total_area": sum(areas), "n": len(areas)}
...
def q3_density(zone_id: str, confidence_min: float = 0.0):
    ...
    return {"zone_id": zone_id, "density_per_km2": count_result["count"] / area if area > 0 else 0}
...
def q4_compare(zone_a: str, zone_b: str, confidence_min: float = 0.0):
    ...
    return {"zone_a": zone_a, "density_a": da, "zone_b": zone_b, "density_b": db,
...
def q5_confidence_dist(zone_id: str, bins: int = 5):
    ...
    return [{"bucket": i, "min": float(edges[i]), "max": float(edges[i+1]), "count": int(counts[i])}
            for i in range(bins)]
